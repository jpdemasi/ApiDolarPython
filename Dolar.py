...
if response.status_code == 200:
    ...
    if len(values) >= 2:
        ...
        compra = values[0].strip().replace('$', '').replace(',', '.')  # Elimina el '$' y cambiar la coma por un punto
        venta = values[1].strip().replace('$', '').replace(',', '.')  # Elimina el '$' y cambiar la coma por un punto
